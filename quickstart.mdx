...
<Steps>
  <Step title="Create Your Account">
    ...
    4. You're in! **Every new account gets 500 free credits** to start testing the API immediately.
  ...
  <Step title="Purchase Credits (Optional)">
  ...
  <Step title="Make Your First Request">
    Let's fetch today's soccer events with odds and AI recommendations!

    <CodeGroup>
    ```bash cURL
    curl -X GET "https://api.mrdoge.co/odds/events?sport=soccer&status=upcoming&status=live&locale=en" \
    ...
    async function getEvents() {
      ...
      console.log(`Found ${data.length} upcoming soccer matches`);
      return data;
    }
    ...
    def get_events():
        url = 'https://api.mrdoge.co/odds/events'
        ...
        print(f"Found {len(events)} upcoming soccer matches")
        return events
    ...
    $url = 'https://api.mrdoge.co/odds/events?sport=soccer&status=upcoming&status=live&locale=en';
    ...
    echo "Found " . count($events) . " upcoming soccer matches\n";
    ?>
    ```
    </CodeGroup>
...
Here's what you get back from the `/odds/events` endpoint:
...
[
  {
    ...
    "homeTeam": {
      "id": "team_456",
      "name": "Arsenal",
      "logo": "https://api.mrdoge.co/assets/teams/arsenal.png"
    },
    "awayTeam": {
      "id": "team_789",
      "name": "Chelsea",
      "logo": "https://api.mrdoge.co/assets/teams/chelsea.png"
    },
    "startTime": "2025-11-05T20:00:00Z",
    "competition": {
      "id": "comp_123",
      "name": "Premier League",
      "logo": "https://api.mrdoge.co/assets/competitions/premier-league.png"
    },
    "region": {
      "id": "region_01",
      "name": "England",
...
<CardGroup cols={2}>
  <Card title="Explore All Endpoints" icon="map" href="/api-reference/odds/events">
    Browse our complete API reference with 42+ endpoints for odds, live data, and AI predictions
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="Slow Response Times" icon="clock">
    ...
    **Solutions**:
    ...
    - Use the `/odds/prerender` endpoint for static/cached data
