...
description: API keys for servers and short-lived JWTs for browsers, in full detail.
icon: KeyRound
---
...
The SDK POSTs to `authEndpoint` automatically whenever it needs a fresh
token: first connect, after expiry, after a reconnect.
...
`UnauthorizedError` to your code. You'll typically never see this since the
