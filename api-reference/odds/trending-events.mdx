...
Events are ranked by view count and include basic event details with key markets. This is a lightweight endpoint optimized for speed - for full AI analysis, use the [Event Details](/api-reference/odds/event-details) endpoint.
...
<Note>
  **Lightweight Endpoint**: This endpoint does NOT include:
  ...
  For complete event data with AI analysis, use the [Event Details](/api-reference/odds/event-details) endpoint.
</Note>
...
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/trending-events?sport=soccer&locale=en"
```

```bash Get Completed Events (Most Viewed Today)
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/trending-events?status=completed&sport=soccer&locale=en"
```
