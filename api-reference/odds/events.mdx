...
<Warning>
**For real-time live data**: Use the [Get Live Events](/api-reference/odds/live-events) endpoint (2 credits) instead of `status=live` on this endpoint. The `/live-events` endpoint provides real-time scores, odds, and statistics with sub-second response times.
...
For full event details including markets and odds, use the [Get Event Details](/api-reference/odds/event-details) endpoint.
...
<ParamField query="status" type="string" default="upcoming,live">
  ...
  <Warning>
    **Using `status=live`?** This returns a static list of live events without real-time data. For live scores, odds, and statistics, use the dedicated [/live-events](/api-reference/odds/live-events) endpoint instead.
  </Warning>
</ParamField>
...
<ParamField query="limit" type="string" default="100">
  ...
  Use with `afterDate` for cursor-based pagination to fetch additional results.
</ParamField>

<ParamField query="afterDate" type="string">
  **Pagination cursor**: Fetch events starting after this datetime (ISO 8601 format)

  Example: `2025-11-14T15:30:00Z`

  <Note>
    **How pagination works**:
    1. First request: `GET /events?limit=100` → Returns 100 events
    2. Get the last event's `startDateTime` from response
    3. Next request: `GET /events?limit=100&afterDate=2025-11-14T15:30:00Z` → Returns next 100 events after that time
    4. Repeat until no more results

    This is **cursor-based pagination** - more efficient than offset-based pagination for time-series data.
  </Note>
</ParamField>
...
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/events?competitionId=1&date=2025-11-08&limit=10&status=upcoming&status=live&status=completed&locale=en"
```

```javascript JavaScript
const response = await fetch('https://api.mrdoge.co/odds/events?competitionId=1&date=2025-11-08&limit=10&status=upcoming&status=live&status=completed&locale=en', {
  headers: {
...
# Week 1: Nov 1-7
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/events?startDate=2025-11-01&endDate=2025-11-07&limit=100&locale=en"

# Week 2: Nov 8-14
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/events?startDate=2025-11-08&endDate=2025-11-14&limit=100&locale=en"
```
...
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/events?competitionId=847&limit=100&locale=en"

# Get La Liga events
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/events?competitionId=29&limit=100&locale=en"
```
...
# Get English events
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/events?regionId=6&limit=100&locale=en"

# Get Spanish events
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/events?regionId=5&limit=100&locale=en"
```
...
<CardGroup cols={2}>
  ...
  <Card title="Live Event List" icon="list">
    ```bash
    ?status=live&locale=en
    ```
    **Note**: Returns list only, no live data. Use `/live-events` for real-time stats.
  </Card>

  <Card title="Completed Events" icon="check-circle">
    ```bash
    ?status=completed&date=2025-11-08&locale=en
    ```
    Includes final scores and stats
  </Card>
