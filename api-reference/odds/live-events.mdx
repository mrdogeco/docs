...
**Why use this endpoint instead of `/events?status=live`?**
...
**Key Difference**: `/events?status=live` returns a static list without stats/odds. This endpoint returns **real-time live data** with scores and odds updated every 30 seconds.
...
<Note>
  **Migration from `/events?status=live`**:

  If you're currently using `/events?status=live`, you're only getting a **static list** of event IDs and names without real-time data. That endpoint:
...
The response format is identical to the `/events` endpoint, with the following fields:
...
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/live-events?locale=en"
```

```bash cURL - Filter by Sport
curl -H "Authorization: Bearer sk_live_..." \
  "https://api.mrdoge.co/odds/live-events?sport=soccer&locale=en"
```

```javascript JavaScript
const response = await fetch('https://api.mrdoge.co/odds/live-events?locale=en', {
  headers: {
...
response = requests.get(
    'https://api.mrdoge.co/odds/live-events',
    headers=headers,
...
If you're currently using `/events?status=live`, here's how to migrate:

### Before (General Events Endpoint)
```javascript
// Static list, no live data
const response = await fetch(
  'https://api.mrdoge.co/odds/events?status=live&locale=en',
...
| Scenario | Use `/live-events` | Use `/events?status=live` |
|----------|-------------------|---------------------------|
