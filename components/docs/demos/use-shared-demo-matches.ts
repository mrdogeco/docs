 ...
 *   two fallback resolvers below — matches that need a match actually in
 *   progress (real events, real stats).
 * - upcoming: Match Highlight's upcoming example, Bet Slip's single-game
 *   example, Match Card's "with odds" upcoming slot — need a stable match
 *   that won't change mid-read.
