...
- `/odds/events` - Get events with odds (supports `status=live` for live events)
- `/odds/events/:eventId` - Single event details
- `/odds/teams` - Search teams
- `/ai/betting-recommendations` - AI recommendations (filtered)
...
- `/odds/health` - API health check
- `/odds/regions` - Available regions/countries
- `/odds/competitions` - List competitions
- `/odds/stats` - System statistics

### Premium Endpoints (2-10 Credits)

Advanced features cost more:

- `/odds/odds-history` - **2 credits** - Historical odds data
- `/odds/events/:id/live-odds` - **2 credits** - Current live odds
- `/ai/value-opportunities` - **5 credits** - Best value bets with edge analysis
- `/ai/mr-doge-picks` - **10 credits** - Featured parlay picks with AI analysis
...
**Example in code:**
```javascript
const response = await fetch('https://api.mrdoge.co/odds/events?locale=en', {
...
<CardGroup cols={2}>
  ...
  <Card title="Use Free Endpoints" icon="gift">
    Leverage free endpoints like `/odds/regions` and `/odds/stats` for reference data.
  </Card>
...
for (let i = 0; i < 100; i++) {
  await fetch(`https://api.mrdoge.co/odds/events/${eventIds[i]}?locale=en`, {
    headers: { 'Authorization': 'Bearer sk_live_...' }
...
const response = await fetch(
  'https://api.mrdoge.co/odds/events?competitionId=premier-league&limit=100&locale=en',
  { headers: { 'Authorization': 'Bearer sk_live_...' } }
); // 1 request = 1 credit!
```
</CodeGroup>
